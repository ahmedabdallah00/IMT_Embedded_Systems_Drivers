...
void LCD_SendCommand(u8 command)
{
...
#if MODE_4BIT
	u8 Local_Port = DIO_GetPortValue(LCD_DATA_PORT);
	u8 Local_Temp=0;
	...
	DIO_SetPinValue(LCD_CTRL_PORT ,LCD_RW_PIN , DIO_LOW);
	//3-Set DB pins
	Local_Temp = (command & 0xf0)| (Local_Port & 0x0f);
	DIO_SetPortValue(LCD_DATA_PORT , Local_Temp);
	//4-Send an enable pulse
	DIO_SetPinValue(LCD_CTRL_PORT,LCD_EN_PIN,DIO_HIGH);
	_delay_ms(1);
	DIO_SetPinValue(LCD_CTRL_PORT,LCD_EN_PIN,DIO_LOW);
	_delay_ms(1);
	Local_Temp = (command<<4)| (Local_Port & 0x0f);
	DIO_SetPortValue(LCD_DATA_PORT , Local_Temp);
	//4-Send an enable pulse
	DIO_SetPinValue(LCD_CTRL_PORT,LCD_EN_PIN,DIO_HIGH);
	_delay_ms(1);
...
void LCD_SendChar(char c)
{
	...
	//1-Set RS pin --> High
		...
		DIO_SetPinValue(LCD_CTRL_PORT ,LCD_RW_PIN , DIO_LOW);
		//3-Set DB pins
		Local_Temp = (c & 0xf0)| (Local_Port & 0x0f);
		DIO_SetPortValue(LCD_DATA_PORT , Local_Temp);
		//4-Send an enable pulse
		DIO_SetPinValue(LCD_CTRL_PORT,LCD_EN_PIN,DIO_HIGH);
		_delay_ms(1);
		DIO_SetPinValue(LCD_CTRL_PORT,LCD_EN_PIN,DIO_LOW);
		_delay_ms(1);
		Local_Temp = (c <<4)| (Local_Port & 0x0f);
		DIO_SetPortValue(LCD_DATA_PORT , Local_Temp);
		//4-Send an enable pulse
		DIO_SetPinValue(LCD_CTRL_PORT,LCD_EN_PIN,DIO_HIGH);
		_delay_ms(1);
